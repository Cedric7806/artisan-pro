...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const documentResult = await query<StoredDocumentData>("SELECT * FROM devis WHERE id = $1 AND entreprise_id = $2", [id, user.entreprise_id]);
  const documentData = documentResult.rows[0];
  if (!documentData) return new NextResponse("Introuvable", { status: 404 });
  const lines = await query<DocumentLine>("SELECT * FROM lignes_devis WHERE devis_id = $1 ORDER BY ordre", [id]);
  const pdf = await buildBusinessPdf({ ...documentData, type: "devis" }, lines.rows);
